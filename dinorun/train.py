...
def train(model, game_state, observe=False):
    ...
    x_t, r_0, terminal = game_state.get_state(
        init_action)  # get next step after action
    ...
    while True:  # endless running
        ...
        if t % settings[
            'frames_per_action'] == 0:  # parameter to skip frames for actions
        ...
        if t > OBSERVE:
            ...
            mini_batch = random.sample(replay_memory,
                                       settings['batch_size'])
            inputs = np.zeros((settings['batch_size'], s_t.shape[1],
                               s_t.shape[2], s_t.shape[3]))
            # 32, 20, 40, 4
            targets = np.zeros(
                (inputs.shape[0], settings['num_actions']))  # 32, 2
            ...
            for i in range(0, len(mini_batch)):
                ...
                terminal = mini_batch[i][
                    4]  # whether the agent died or survided due the action
                ...
                if terminal:
                    targets[
                        i, action_t] = reward_t  # if terminated, only equals reward
                else:
                    targets[i, action_t] = reward_t + settings[
                        'gamma'] * np.max(Q_sa)
        ...
        if t % 1000 == 0:
            ...
            save_obj(epsilon,
                     "epsilon")  # cache epsilon to avoid repeated randomness in actions
